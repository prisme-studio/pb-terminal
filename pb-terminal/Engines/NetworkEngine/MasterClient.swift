...
class MasterClient {
	...
	private func runLoop() {
		while(isOpen) {
			...
			// Decode the message
			guard let datagram = try? Network_Messages_Datagram(serializedData: Data(data)) else {
	...
	public func close() {
		...
		var datagram = Network_Messages_Datagram();
		datagram.type = DatagramType.close.rawValue;
...
extension MasterClient {
	@objc func requestStatus() {
		var datagram = Network_Messages_Datagram();
		datagram.type = DatagramType.status.rawValue;
	...
	func requestLayoutList() {
		var datagram = Network_Messages_Datagram();
		datagram.type = DatagramType.layoutList.rawValue;

		send(datagram);
	}

	func requestLayout(name: String) {
		var layoutName = Pb_Messages_LayoutName();
		layoutName.name = name;

		var datagram = Network_Messages_Datagram();
		datagram.type = DatagramType.layoutOpen.rawValue;
	...
	func createLayout(withName name: String) {
		...
		var datagram = Network_Messages_Datagram();
		datagram.type = DatagramType.layoutCreate.rawValue;
		datagram.data = try! Google_Protobuf_Any(message: layoutName);
	...
	func deleteLayout(withName name: String) {
		...
		var datagram = Network_Messages_Datagram();
		datagram.type = DatagramType.layoutDelete.rawValue;
		datagram.data = try! Google_Protobuf_Any(message: layoutName);
	...
	func updateLayout(updatedLayout layout: Layout) {
		var datagram = Network_Messages_Datagram();
		datagram.type = DatagramType.layoutUpdate.rawValue;
	...
	func setCalibrationDevices(_ deviceA: String, _ deviceB: String) {
		var message = Pb_Messages_CalibrationDevices();
		message.deviceA = deviceA;
		message.deviceB = deviceB;

		var datagram = Network_Messages_Datagram();
		datagram.type = DatagramType.calibrationSet.rawValue;
...
extension MasterClient {
	...
	func ping() {
		...
		var ping = Network_Messages_Ping();
		ping.time = UInt64(Date().timeIntervalSince1970 * 1000.0);

		var datagram = Network_Messages_Datagram();
		datagram.type = DatagramType.ping.rawValue;
	...
	func onPing(_ data: SwiftProtobuf.Google_Protobuf_Any) {
		...
		var datagram = Network_Messages_Datagram();
		datagram.type = DatagramType.pong.rawValue;
		datagram.data = data;
	...
	func onPong(_ data: SwiftProtobuf.Google_Protobuf_Any) {
		// Decode the message
		let pong = try! Network_Messages_Ping(unpackingAny: data);
