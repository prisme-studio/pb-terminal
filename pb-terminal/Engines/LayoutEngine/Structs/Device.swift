...
class Device {
	...
	/// - Parameter message: -
	convenience init(fromMessage message: Pb_Messages_Device) {
		self.init()
	...
	func asMessage() -> Pb_Messages_Device {
		var message = Pb_Messages_Device()
		message.name = name;
		message.uid = uid;
